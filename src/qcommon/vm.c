...
int QDECL VM_DllSyscall( int arg, ... ) {
  #if (((defined __linux__) && (defined __powerpc__)) || (defined __mips__))
...
#ifdef PSP_VM
#error vm not available -> only static
	vmHeader_t	*header;
	int			length;
	int			dataLength;	
	char		filename[MAX_QPATH];
...
#error dynamic not available -> only static
...
#error vm not available -> only static
	...
	return vm;
#endif
#ifndef PSP_DYNAMIC_LINKING
#ifndef PSP_VM
	extern int vmMain ( int callNum, ... ); 
	
	vm->entryPoint = vmMain;
	Com_Printf("VM_Create(%s) found **vmMain** at  %p  \n",module,vmMain); 
	vm->dllHandle = (void *) 1; //some strange but works :)
	void dllEntry( int (QDECL *syscallptr)( int arg,... ) ) ;
	dllEntry(VM_DllSyscall);
	Com_Printf ( "VM_Create(%s) succeeded!\n", module );
...
int	QDECL VM_Call( vm_t *vm, int callnum, ... ) {
	...
	if ( vm->entryPoint ) {
		...
		Com_Printf("FATAL_ERROR : vm->entryPoint not found !\n");
