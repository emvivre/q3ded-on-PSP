...
#define MIN_DEDICATED_COMZONEMEGS 1
#define MIN_COMHUNKMEGS 56
#define DEF_DEDICATED_COMHUNKBYTES "13631488"
#define DEF_COMHUNKMEGS "56"
#define DEF_DEDICATED_COMZONEBYTES "2726297"
...
typedef struct heap_mem_header{
	void * ptr;
	int size;
	struct heap_mem_header_t *next;
	struct heap_mem_header_t *prev;
}heap_mem_header_t;

/*
=============================
displayfree
Montre ce qu il reste dans le heap
=============================
*/

void displayfree(void * ptr, unsigned int ptrsize ){
	
	
	int size ;

        //#ifdef DEBUG		
	//printf("** Agencement du heap **\n");
	//printf("    Base    |    Size    |    Free  \n"); 
	//#endif 
	if ( ptr == NULL ){
		printf("ERROR FATAL : ptr is NULL !\n");
		return ;
	}
	
	size = ((u32)heap_top - ((u32)ptr + ptrsize) - sizeof(heap_mem_header_t));

	if (size<0)
	  size=0;
	
	if ((size  & (DEFAULT_ALIGNMENT - 1)) != 0)
	  size = ALIGNDW(size, DEFAULT_ALIGNMENT);
	
	//#ifdef DEBUG
	//printf(" 0x%08X | %8u | %8u \n",(unsigned int ) ptr, ptrsize,size);
	//	#endif 
	heap_free = size - 512;
...
void Com_Cpu(){
	cvar_t * cv;
	int pllfreq,cpufreq,busfreq;
	
	cv = Cvar_Get( "pllfreq", "333",0);
	pllfreq = cv->integer;
	
	cv = Cvar_Get( "cpufreq", "333",0);
	cpufreq = cv->integer;

	cv = Cvar_Get( "busfreq", "166",0);
	busfreq = cv->integer;
...
memzone_t	*mainzone;
//block_t         mainzoneblock;

// we also have a small zone for small allocations that would only
// fragment the main zone (think of cvar and cmd strings)
memzone_t	*smallzone;
//block_t         smallzoneblock;
...
void Com_InitSmallZoneMemory( void ) {
	...
	s_smallZoneTotal = 22 * 1024;
	
	// bk001205 - was malloc
	//smallzone = Com_calloc( s_smallZoneTotal, 1, &smallzoneblock );
	heap_size = sceKernelMaxFreeMemSize();
	smallzone = calloc( s_smallZoneTotal, 1 );
	//smallzone = Com_calloc( s_smallZoneTotal, 1, &h);
	
	if ( !smallzone ) {
		Com_Error( ERR_FATAL, "Small zone data failed to allocate %1.1f megs", (float)s_smallZoneTotal / (1024*1024) );
	}
	heap_top = (void *) ( (u32) smallzone - s_smallZoneTotal + heap_size);
	displayfree(smallzone,s_smallZoneTotal);
...
void Com_InitZoneMemory( void ) {
	...
	cv = Cvar_Get( "com_zoneBytes", DEF_DEDICATED_COMZONEBYTES, CVAR_LATCH | CVAR_ARCHIVE );

	if ( cv->integer < MIN_DEDICATED_COMZONEMEGS*1024*1024 ) {
		s_zoneTotal = 1024 * 1024 * MIN_DEDICATED_COMZONEMEGS;
	} else {
		s_zoneTotal = cv->integer;
	}
	
	printf("Attemp to allocate %i\n",s_zoneTotal);

	// bk001205 - was malloc
	//mainzone = Com_calloc( s_zoneTotal, 1 ,&h);
	mainzone = calloc( s_zoneTotal, 1);
	
	if ( !mainzone ) {
		Com_Error( ERR_FATAL, "Zone data failed to allocate %i bytes", s_zoneTotal);
	}
	displayfree(mainzone,s_zoneTotal);
...
void Com_InitHunkMemory( void ) {
	...
	cv = Cvar_Get( "com_hunkBytes", DEF_DEDICATED_COMHUNKBYTES, CVAR_LATCH | CVAR_ARCHIVE );

	// if we are not dedicated min allocation is 56, otherwise min is 1
	
	if (com_dedicated && com_dedicated->integer) {
		nMinAlloc = MIN_DEDICATED_COMHUNKMEGS*1024*1024;
		pMsg = "Minimum com_hunkBytes for a dedicated server is %i, allocating %i bytes.\n";
	}
	else {
		nMinAlloc = MIN_COMHUNKMEGS*1024*1024;
		pMsg = "Minimum com_hunkBytes is %i, allocating %i bytes.\n";
	}
       
	if ( cv->integer < nMinAlloc ) {
		s_hunkTotal = nMinAlloc;
		Com_Printf(pMsg, nMinAlloc, s_hunkTotal );
	} else {
		s_hunkTotal = cv->integer;
	}
	
	
	cv =  Cvar_Get( "com_useallrestforHunk", "0",0);
	if (cv->integer == 1)
		//use all memory available
		s_hunkTotal = heap_free;
	

	// bk001205 - was malloc
	
	printf("Attempt to allocate %d bytes for Hunk Data\n",s_hunkTotal);
	//s_hunkData = Com_calloc( s_hunkTotal + 31, 1, &h );
	s_hunkData = calloc( s_hunkTotal + 31, 1);
	
	if ( !s_hunkData ) {
		Com_Error( ERR_FATAL, "Hunk data failed to allocate %i bytes", s_hunkTotal );
	}
	displayfree(s_hunkData,s_hunkTotal + 31);
...
void *Hunk_Alloc( int size, ha_pref preference ) {
	...
	if ( hunk_low.temp + hunk_high.temp + size > s_hunkTotal ) {
		...
		Hunk_SmallLog();
#endif
		rd_buffer = NULL;//necessary if we want display something to psp side
		Com_Error( ERR_DROP, "Hunk_Alloc failed on %i; missing : %d bytes ", size, hunk_low.temp + hunk_high.temp + size - s_hunkTotal );
	}
...
void *Hunk_AllocateTempMemory( int size ) {
	...
	if ( hunk_temp->temp + hunk_permanent->permanent + size > s_hunkTotal ) {
		Com_Error( ERR_DROP, "Hunk_AllocateTempMemory: failed on %i ; missing : %d", size, hunk_temp->temp + hunk_permanent->permanent + size -  s_hunkTotal );
	}
...
void Com_Frame( void ) {
	...
	if ( setjmp (abortframe) ) {
		Sys_Error ("Error during frame computing");
	...
	#if 0 //not for a dedicated server
	//
	// client system
	//
	if ( !com_dedicated->integer ) {
...
#if ((!id386) && (!defined __i386__)) // rcg010212 - for PPC or MIPS
